"""Tenant-isolation proof for payments (invariant 1)."""
...
class PaymentRowLevelSecurityTests(TestCase):
    @staticmethod
    def _invoice(tenant, phone, bed_number='101-A'):
        ...
        with tenant_context(tenant.id):
            invoice = Invoice.objects.create(
                ...
                status=Invoice.Status.ISSUED, issue_date=date(2026, 7, 1),
            )
    ...
    def setUp(self):
        ...
        self.invoice_b = self._invoice(self.tenant_b, '9000000002')
        with tenant_context(self.tenant_a.id):
            self.payment_a = Payment.objects.create(
                tenant_id=self.tenant_a.id, invoice=self.invoice_a, amount='2000.00',
                payment_date=date(2026, 7, 5), payment_mode='cash',
            )
        with tenant_context(self.tenant_b.id):
            Payment.objects.create(
                tenant_id=self.tenant_b.id, invoice=self.invoice_b, amount='3000.00',
                payment_date=date(2026, 7, 5), payment_mode='cash',
            )
    ...
    def test_tenant_context_sees_only_own_payments(self):
        with tenant_context(self.tenant_a.id):
            ...
            self.assertEqual(Payment.objects.first().pk, self.payment_a.pk)
            ...
            with self.assertRaises(DatabaseError):
                with transaction.atomic():
                    Payment.objects.create(
                        ...
                        amount='1.00', payment_date=date(2026, 7, 5), payment_mode='cash',
                    )
